...
public partial class MainScene : Node
{
	...
	public void OnConnectorsSizeDifference(Connector source, Connector destination)
	{
		ConnectionWire? wire = Wires.FirstOrDefault(p => p.Source == source && p.Destination == destination);
		if (wire == null)
		{
			GD.PrintErr("Attempted to display connection error but nodes have no wire connecting them");
			return;
		}
		wire.IsDisplayingValidConnection = false;
	}
	...
	private T? AddLogicNode<T>(PackedScene? prefab) where T : LogicNode
	{
		...
		node.OnIncompatibleSizesDetected += OnConnectorsSizeDifference;
		return node;
	}
